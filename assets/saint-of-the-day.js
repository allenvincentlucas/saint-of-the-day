...
async function loadManifest() {
  ...
  const all = (data.saints || []).filter(s => s.slug); // ignore malformed entries
  // Only ever return saints that are actually released. This is the single
  // choke point for the "bulk-create now, reveal automatically later"
  // feature: everything downstream (hero, week strip, archive) only ever
  // sees released saints, so there's no separate place to remember to
  // filter.
  return all.filter(isReleased);
}

/**
 * A saint is "released" once local midnight of their release_date has
 * passed. Until then, they're invisible everywhere on the site — this is
 * what lets a creator commit a whole batch of future saint pages + manifest
 * entries at once without them leaking early.
 *
 * release_date falls back to `added` for entries that don't set it
 * explicitly, so existing manifest entries keep working unchanged.
 */
function isReleased(saint) {
  const raw = saint.release_date || saint.added;
  if (!raw) return true; // no scheduling info at all -> show it
  const releaseAt = new Date(`${raw}T00:00:00`);
  if (isNaN(releaseAt.getTime())) return true; // malformed date -> fail open, don't hide silently
